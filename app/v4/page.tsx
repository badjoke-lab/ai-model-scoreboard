    ...
    for (let i = 0; i < rankings.length; i++) {
      ...
      const categories =
        e.scores && typeof e.scores === "object"
          ? ((e.scores as { categories?: Record<string, unknown> }).categories ?? {})
          : {};
      for (const dimension of V4_DIMENSIONS) {
        if (!isFiniteNumber(categories[dimension.key])) {
          fatal.push(
            `rankings.json: entry[${i}].scores.categories.${dimension.key} is missing/invalid`
          );
          break;
        }
      }
      ...
      <header className="space-y-3">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div className="space-y-2">
            ...
            <p className="text-sm text-slate-400">Evidence-first scoring for AI models.</p>
            ...
            <p className="mt-2 max-w-3xl text-sm text-slate-400">
              ...
              Total blends performance, safety, adoption, openness, and cost scores.
            </p>
          </div>
